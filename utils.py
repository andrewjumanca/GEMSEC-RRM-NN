...
def load_data(csv, auto_convert=False, arr_cols=None, names=None):
    ...
    Can take a list of columns containing arrays casted as strings to convert
    them back to arrays as a new column. Can also take a boolean to look for
    arrays casted to strings and automatically convert them. Can also take a list
    of names for converted columns listed in arr_cols.
    ...
    if arr_cols != None:
        for i, col in enumerate(arr_cols):
            ...
            elif type(data[col][0]) == str and data[col][0][0] == '[':
                name = '' + col + '_as_array'
                if names != None:
                    name = names[i]
                data[name] = data[col].apply(convert_array)
            else: # column is not a string
